    ...
    **unknown_options,
    ...
    while m.jac_mag >= gtol:
        ...
        if hessp is not None:
            predicted_reduction_alt = 0.5 * np.dot(p, hessp(x, p))
        else:
            predicted_reduction_alt = 0.5 * np.dot(p, (hess(x) @ p[:, None])[:, 0])

        print("trust_radius", trust_radius)
        print("actual_reduction", actual_reduction)
        print("predicted_reduction", predicted_reduction)
        print("predicted_reduction_alt", predicted_reduction_alt)
        print("magp", np.linalg.norm(p))
        print("magg", m.jac_mag)
